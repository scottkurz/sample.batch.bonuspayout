...
#     Example usage: docker build --build-arg ARG ADD_MP_HEALTH=false ...
...
RUN if [ "$ENABLE_OPENJ9_SCC" = "false" ]; then \
        ENABLE_OPENJ9_SCC=false; \
    elif [ "$ENABLE_OPENJ9_SCC" != "true" ]; then \
        echo "Invalid ENABLE_OPENJ9_SCC value: $ENABLE_OPENJ9_SCC. Valid values are \"true\" | \"false\". Using the default of true."; \
    fi
